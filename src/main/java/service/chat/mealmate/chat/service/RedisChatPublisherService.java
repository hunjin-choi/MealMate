...
public class RedisChatPublisherService {
    ...
    public void convertAndSend(RedisChatMessageDto message, String loginId, Long mealMateId) {
        ...
        ChatMessage chatMessage = mealMateService.saveChatMessage(message.getMessage(), mealMateId);
        message.setDate(chatMessage.getSentAt());
        // Websocket에 발행된 메시지를 redis로 발행(publish)
        redisTemplate.convertAndSend(channelTopic.getTopic(), message);
    }

    public void convertAndSend(String message, String loginId, String roomId, Long mealMateId, RedisChatMessageDto.MessageType messageType) {
        // 나중에 수정 필요
        // 로그인 회원 정보로 대화명 설정
        if (loginId == null) loginId = "TempNickname";
        RedisChatMessageDto redisChatMessageDto = new RedisChatMessageDto(messageType, roomId, loginId, message);
        // 채팅방 입장시에는 대화명과 메시지를 자동으로 세팅한다.
        ChatMessage chatMessage = mealMateService.saveChatMessage(message, mealMateId);
